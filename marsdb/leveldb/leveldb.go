...
func (db *Database) NewBatch() marsdb.Batch {
	return &batch{
		db: db.db,
		b:  new(leveldb.Batch),
	}
...
func (db *Database) NewBatchWithSize(size int) marsdb.Batch {
	return &batch{
		db: db.db,
...
func (db *Database) NewIterator(prefix []byte, start []byte) marsdb.Iterator {
...
func (b *batch) Replay(w marsdb.KeyValueWriter) error {
	return b.b.Replay(&replayer{writer: w})
}

// replayer is a small wrapper to implement the correct replay methods.
type replayer struct {
	writer  marsdb.KeyValueWriter
	failure error
}
