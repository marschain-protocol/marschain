...
marschain:
	env GO111MODULE=on go build -ldflags="$(ldflags)" -o $(GOBIN)/marschain ./cmd/geth
	@echo "Done building."
	@echo "Run \"$(GOBIN)/marschain\" to launch marschain."

marschain-debug:
	env GO111MODULE=on go build -gcflags="all=-N -l" -ldflags="$(ldflags)" -o $(GOBIN)/marschain-debug ./cmd/geth
	@echo "Done building debug version."
	@echo "Run \"$(GOBIN)/marschain-debug\" to launch marschain."
...
# Linux x64 builds
geth-linux-x64:
	env GO111MODULE=on GOARCH=amd64 GOOS=linux go build -ldflags="$(ldflags)" -o build/bin/geth-linux-x64 ./cmd/geth

marschain-linux-x64:
	env GO111MODULE=on GOARCH=amd64 GOOS=linux go build -ldflags="$(ldflags)" -o build/bin/marschain-linux-x64 ./cmd/geth

marschain-linux-x64-debug:
	env GO111MODULE=on GOARCH=amd64 GOOS=linux go build -gcflags="all=-N -l" -ldflags="$(ldflags)" -o build/bin/marschain-linux-x64-debug ./cmd/geth
