...
import (
	...
	"github.com/ethereum/go-ethereum/consensus/deimos"
...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	...
	case apitypes.ApplicationDeimos.Mime:
		stringData, ok := data.(string)
		if !ok {
			return nil, useEthereumV, fmt.Errorf("input for %v must be an hex-encoded string", apitypes.ApplicationDeimos.Mime)
		}
		deimosData, err := hexutil.Decode(stringData)
		if err != nil {
			return nil, useEthereumV, err
		}
		header := &types.Header{}
		if err := rlp.DecodeBytes(deimosData, header); err != nil {
			return nil, useEthereumV, err
		}
		// The incoming deimos header is already truncated, sent to us with a extradata already shortened
		if len(header.Extra) < 65 {
			// Need to add it back, to get a suitable length for hashing
			newExtra := make([]byte, len(header.Extra)+65)
			copy(newExtra, header.Extra)
		...
		// Get back the rlp data, encoded by us
		sighash, deimosRlp, err := deimosHeaderHashAndRlp(header, api.chainID)
		if err != nil {
			return nil, useEthereumV, err
		}
		messages := []*apitypes.NameValueType{
			{
				Name:  "Deimos header",
				Typ:   "deimos",
				Value: fmt.Sprintf("deimos header %d [0x%x]", header.Number, header.Hash()),
			},
		}
		// Deimos uses V on the form 0 or 1
		useEthereumV = false
		req = &SignDataRequest{ContentType: mediaType, Rawdata: deimosRlp, Messages: messages, Hash: sighash}
...
func deimosHeaderHashAndRlp(header *types.Header, chainId *big.Int) (hash, rlp []byte, err error) {
	if len(header.Extra) < 65 {
		err = fmt.Errorf("deimos header extradata too short, %d < 65", len(header.Extra))
		return
	}
	rlp = deimos.DeimosRLP(header, chainId)
	hash = deimos.SealHash(header, chainId).Bytes()
	return hash, rlp, err
}
