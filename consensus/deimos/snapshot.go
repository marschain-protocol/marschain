...
type Snapshot struct {
	config   *params.DeimosConfig // Consensus engine parameters to fine tune behavior
...
func loadSnapshot(config *params.DeimosConfig, sigCache *lru.ARCCache, db marsdb.Database, hash common.Hash, ethAPI *ethapi.PublicBlockChainAPI) (*Snapshot, error) {
	blob, err := db.Get(append([]byte("deimos-"), hash[:]...))
	if err != nil {
		return nil, err
	}
	snap := new(Snapshot)
...
func (s *Snapshot) store(db marsdb.Database) error {
	blob, err := json.Marshal(s)
	if err != nil {
		return err
	}
	return db.Put(append([]byte("deimos-"), s.Hash[:]...), blob)
}
