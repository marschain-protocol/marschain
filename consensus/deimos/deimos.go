...
// DeimosRLP returns the rlp bytes which needs to be signed for the deimos
...
func DeimosRLP(header *types.Header, chainId *big.Int) []byte {
	b := new(bytes.Buffer)
	encodeSigHeader(b, header, chainId)
	return b.Bytes()
}

// Deimos is the consensus engine of Marschain
type Deimos struct {
	chainConfig *params.ChainConfig  // Chain config
	config      *params.DeimosConfig // Consensus engine configuration parameters for deimos consensus
	genesisHash common.Hash
	db          marsdb.Database // Database to store and retrieve snapshot checkpoints
...
// New creates a marschain Deimos consensus engine.
func New(
	...
	// get deimos config for marschain
	deimosConfig := chainConfig.Deimos

	// Set any missing consensus parameters to their defaults
	if deimosConfig != nil && deimosConfig.Epoch == 0 {
		deimosConfig.Epoch = defaultEpochLength
	}
...
func (p *Deimos) IsSystemTransaction(tx *types.Transaction, header *types.Header) (bool, error) {
	// deploy a contract
	if tx.To() == nil {
		return false, nil
	}
...
func (p *Deimos) IsSystemContract(to *common.Address) bool {
	if to == nil {
		return false
	}
...
func (p *Deimos) Author(header *types.Header) (common.Address, error) {
	return header.Coinbase, nil
}

// VerifyHeader checks whether a header conforms to the consensus rules.
func (p *Deimos) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header, seal bool) error {
...
func (p *Deimos) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
...
func (p *Deimos) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (p *Deimos) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (p *Deimos) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
func (p *Deimos) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func (p *Deimos) VerifySeal(chain consensus.ChainReader, header *types.Header) error {
...
// from.
func (p *Deimos) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
// with.
func (p *Deimos) Authorize(val common.Address, signFn SignerFn, signTxFn SignerTxFn) {
	p.lock.Lock()
	defer p.lock.Unlock()
...
func (p *Deimos) Delay(chain consensus.ChainReader, header *types.Header, leftOver *time.Duration) *time.Duration {
	number := header.Number.Uint64()
	snap, err := p.snapshot(chain, number-1, header.ParentHash, nil)
	if err != nil {
		return nil
...
func (p *Deimos) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	// Sign all the things!
	sig, err := signFn(accounts.Account{Address: val}, accounts.MimetypeDeimos, DeimosRLP(header, p.chainConfig.ChainID))
	if err != nil {
		return err
	}
...
func (p *Deimos) shouldWaitForCurrentBlockProcess(chain consensus.ChainHeaderReader, header *types.Header, snap *Snapshot) bool {
...
func (p *Deimos) EnoughDistance(chain consensus.ChainReader, header *types.Header) bool {
...
// current signer.
func (p *Deimos) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
...
func (p *Deimos) SealHash(header *types.Header) common.Hash {
	return SealHash(header, p.chainConfig.ChainID)
}

// APIs implements Deimos consensus.Engine, returning the user facing RPC API to query snapshot.
func (p *Deimos) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return []rpc.API{{
		Namespace: "deimos",
		Version:   "1.0",
		Service:   &API{chain: chain, deimos: p},
		Public:    false,
	}}
}

// Close implements consensus.Engine. It's a noop for deimos as there are no background threads.
func (p *Deimos) Close() error {
	return nil
}
...
func (p *Deimos) getCurrentValidators(blockHash common.Hash, blockNumber *big.Int) ([]common.Address, error) {
	// block
...
// get system message
func (p *Deimos) getSystemMessage(from, toAddress common.Address, data []byte, value *big.Int) callmsg {
	return callmsg{
		ethereum.CallMsg{
			From:     from,
...
func (p *Deimos) applyTransaction(
	msg callmsg,
	state *state.StateDB,
	header *types.Header,
...
func (p *Deimos) backOffTime(snap *Snapshot, header *types.Header, val common.Address) uint64 {
	if snap.inturn(val) {
		return 0
	} else {
