...
// Package marstats implements the network stats reporting service.
package marstats
...
import (
	...
	ethproto "github.com/ethereum/go-ethereum/mars/protocols/eth"
...
// backend encompasses the bare-minimum functionality needed for marstats reporting
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the marstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to marstats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to marstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to marstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to marstats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to marstats")
