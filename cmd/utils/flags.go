...
import (
	...
	"github.com/ethereum/go-ethereum/mars"
	ethcatalyst "github.com/ethereum/go-ethereum/mars/catalyst"
	...
	"github.com/ethereum/go-ethereum/mars/tracers"
	"github.com/ethereum/go-ethereum/marsdb"
	"github.com/ethereum/go-ethereum/marstats"
...
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *mars.Ethereum) {
	if cfg.SyncMode == downloader.LightSync {
	...
	backend, err := mars.New(stack, cfg)
	if err != nil {
		Fatalf("Failed to register the Ethereum service: %v", err)
	}
	if cfg.LightServ > 0 {
...
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := marstats.New(stack, backend, backend.Engine(), url); err != nil {
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node, readonly, disableFreeze bool) marsdb.Database {
	var (
		cache   = ctx.GlobalInt(CacheFlag.Name) * ctx.GlobalInt(CacheDatabaseFlag.Name) / 100
		handles = MakeDatabaseHandles()

		err     error
		chainDb marsdb.Database
	)
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *core.BlockChain, chainDb marsdb.Database) {
	var err error
