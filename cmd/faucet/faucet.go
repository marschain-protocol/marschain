...
import (
	...
	"github.com/ethereum/go-ethereum/marsclient"
...
type faucet struct {
	...
	client *marsclient.Client  // Client connection to the Ethereum chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*enode.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte, bep2eInfos map[string]bep2eInfo) (*faucet, error) {
	...
	// Assemble the marstats monitoring and reporting service'
	if stats != "" {
		if err := marstats.New(stack, lesBackend.ApiBackend, lesBackend.Engine(), stats); err != nil {
			return nil, err
		}
	}
	...
	client := marsclient.NewClient(api)
