...
func (c *Conn) handshake() error {
	...
	case *Hello:
		...
		if c.negotiatedProtoVersion == 0 {
			return fmt.Errorf("could not negotiate mars protocol (remote caps: %v, local mars version: %v)", msg.Caps, c.ourHighestProtoVersion)
		}
...
// negotiateEthProtocol sets the Conn's mars protocol version to highest
...
loop:
	...
	// make sure mars protocol version is set for negotiation
	if c.negotiatedProtoVersion == 0 {
		return nil, fmt.Errorf("mars protocol version must be set in Conn")
	}
	if status == nil {
