...
func InitDatabaseFromFreezer(db marsdb.Database) {
	...
	for i := uint64(0) + offset; i < frozen+offset; i++ {
		...
		for j, h := range data {
			...
			if batch.ValueSize() > marsdb.IdealBatchSize {
...
func iterateTransactions(db marsdb.Database, from uint64, to uint64, reverse bool, interrupt chan struct{}) chan *blockTxHashes {
...
// signal received.
func indexTransactions(db marsdb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
...
// There is a passed channel, the whole procedure will be interrupted if any
// signal received.
func IndexTransactions(db marsdb.Database, from uint64, to uint64, interrupt chan struct{}) {
...
func indexTransactionsForTesting(db marsdb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
...
// There is a passed channel, the whole procedure will be interrupted if any
// signal received.
func unindexTransactions(db marsdb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
	// short circuit for invalid range
	if offset := db.AncientOffSet(); offset > from {
		from = offset
	}
...
// There is a passed channel, the whole procedure will be interrupted if any
// signal received.
func UnindexTransactions(db marsdb.Database, from uint64, to uint64, interrupt chan struct{}) {
...
func unindexTransactionsForTesting(db marsdb.Database, from uint64, to uint64, interrupt chan struct{}, hook func(uint64) bool) {
