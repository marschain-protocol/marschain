...
func (f *freezer) ReadAncients(fn func(marsdb.AncientReader) error) (err error) {
	f.writeLock.RLock()
	defer f.writeLock.RUnlock()
	return fn(f)
}
...
func (f *freezer) ModifyAncients(fn func(marsdb.AncientWriteOp) error) (writeSize int64, err error) {
	if f.readonly {
...
func (f *freezer) freeze(db marsdb.KeyValueStore) {
...
func (f *freezer) freezeRange(nfdb *nofreezedb, number, limit uint64) (hashes []common.Hash, err error) {
	...
	_, err = f.ModifyAncients(func(op marsdb.AncientWriteOp) error {
...
func gcKvStore(db marsdb.KeyValueStore, ancients []common.Hash, first uint64, frozen uint64, start time.Time) {
