...
import (
	...
	"github.com/ethereum/go-ethereum/marsdb/leveldb"
	"github.com/ethereum/go-ethereum/marsdb/memorydb"
)

// freezerdb is a database wrapper that enabled freezer data retrievals.
type freezerdb struct {
	marsdb.KeyValueStore
	marsdb.AncientStore
	diffStore marsdb.KeyValueStore
}
...
func (frdb *freezerdb) DiffStore() marsdb.KeyValueStore {
	return frdb.diffStore
}

func (frdb *freezerdb) SetDiffStore(diff marsdb.KeyValueStore) {
	if frdb.diffStore != nil {
...
func (db *nofreezedb) ModifyAncients(func(marsdb.AncientWriteOp) error) (int64, error) {
	return 0, errNotSupported
}
...
func (db *nofreezedb) DiffStore() marsdb.KeyValueStore {
	return db.diffStore
}

func (db *nofreezedb) SetDiffStore(diff marsdb.KeyValueStore) {
	db.diffStore = diff
}

func (db *nofreezedb) ReadAncients(fn func(reader marsdb.AncientReader) error) (err error) {
...
func NewDatabase(db marsdb.KeyValueStore) marsdb.Database {
	return &nofreezedb{KeyValueStore: db}
}

// NewFreezerDb only create a freezer without statedb.
func NewFreezerDb(db marsdb.KeyValueStore, frz, namespace string, readonly bool, newOffSet uint64) (*freezer, error) {
...
// storage.
func NewDatabaseWithFreezer(db marsdb.KeyValueStore, freezer string, namespace string, readonly, disableFreeze, isLastOffset, pruneAncientData, skipCheckFreezerType bool) (marsdb.Database, error) {
	var offset uint64
...
func NewMemoryDatabase() marsdb.Database {
...
func NewMemoryDatabaseWithCap(size int) marsdb.Database {
	return NewDatabase(memorydb.NewWithCap(size))
}

// NewLevelDBDatabase creates a persistent key-value database without a freezer
// moving immutable chain segments into cold storage.
func NewLevelDBDatabase(file string, cache int, handles int, namespace string, readonly bool) (marsdb.Database, error) {
...
// freezer moving immutable chain segments into cold storage.
func NewLevelDBDatabaseWithFreezer(file string, cache int, handles int, freezer string, namespace string, readonly, disableFreeze, isLastOffset, pruneAncientData, skipCheckFreezerType bool) (marsdb.Database, error) {
...
func AncientInspect(db marsdb.Database) error {
...
func InspectDatabase(db marsdb.Database, keyPrefix, keyStart []byte) error {
	...
	for it.Next() {
		...
		case bytes.HasPrefix(key, []byte("deimos-")) && len(key) == 7+common.HashLength:
			deimosSnaps.Add(size)
	...
	stats := [][]string{
		...
		{"Key-Value store", "Deimos snapshots", deimosSnaps.Size(), deimosSnaps.Count()},
