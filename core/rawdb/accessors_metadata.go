...
func ReadDatabaseVersion(db marsdb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db marsdb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db marsdb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db marsdb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func PushUncleanShutdownMarker(db marsdb.KeyValueStore) ([]uint64, uint64, error) {
...
func PopUncleanShutdownMarker(db marsdb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
...
func UpdateUncleanShutdownMarker(db marsdb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
...
func ReadTransitionStatus(db marsdb.KeyValueReader) []byte {
	data, _ := db.Get(transitionStatusKey)
	return data
}

// WriteTransitionStatus stores the eth2 transition status to the database
func WriteTransitionStatus(db marsdb.KeyValueWriter, data []byte) {
...
func ReadOffSetOfCurrentAncientFreezer(db marsdb.KeyValueReader) uint64 {
...
func WriteOffSetOfCurrentAncientFreezer(db marsdb.KeyValueWriter, offset uint64) {
...
func ReadOffSetOfLastAncientFreezer(db marsdb.KeyValueReader) uint64 {
...
func WriteOffSetOfLastAncientFreezer(db marsdb.KeyValueWriter, offset uint64) {
...
func ReadFrozenOfAncientFreezer(db marsdb.KeyValueReader) uint64 {
...
func WriteFrozenOfAncientFreezer(db marsdb.KeyValueWriter, frozen uint64) {
...
func ReadSafePointBlockNumber(db marsdb.KeyValueReader) uint64 {
...
func WriteSafePointBlockNumber(db marsdb.KeyValueWriter, number uint64) {
...
func ReadAncientType(db marsdb.KeyValueReader) uint64 {
...
func WriteAncientType(db marsdb.KeyValueWriter, flag uint64) {
