...
func ReadSnapshotDisabled(db marsdb.KeyValueReader) bool {
	disabled, _ := db.Has(snapshotDisabledKey)
	return disabled
}

// WriteSnapshotDisabled stores the snapshot pause flag.
func WriteSnapshotDisabled(db marsdb.KeyValueWriter) {
...
func DeleteSnapshotDisabled(db marsdb.KeyValueWriter) {
...
func ReadSnapshotRoot(db marsdb.KeyValueReader) common.Hash {
...
// the persisted snapshot.
func WriteSnapshotRoot(db marsdb.KeyValueWriter, root common.Hash) {
...
// invalid.
func DeleteSnapshotRoot(db marsdb.KeyValueWriter) {
...
func ReadAccountSnapshot(db marsdb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(accountSnapshotKey(hash))
	return data
}

// WriteAccountSnapshot stores the snapshot entry of an account trie leaf.
func WriteAccountSnapshot(db marsdb.KeyValueWriter, hash common.Hash, entry []byte) {
...
func DeleteAccountSnapshot(db marsdb.KeyValueWriter, hash common.Hash) {
...
func ReadStorageSnapshot(db marsdb.KeyValueReader, accountHash, storageHash common.Hash) []byte {
	data, _ := db.Get(storageSnapshotKey(accountHash, storageHash))
	return data
}

// WriteStorageSnapshot stores the snapshot entry of an storage trie leaf.
func WriteStorageSnapshot(db marsdb.KeyValueWriter, accountHash, storageHash common.Hash, entry []byte) {
...
func DeleteStorageSnapshot(db marsdb.KeyValueWriter, accountHash, storageHash common.Hash) {
...
func IterateStorageSnapshots(db marsdb.Iteratee, accountHash common.Hash) marsdb.Iterator {
	return db.NewIterator(storageSnapshotsKey(accountHash), nil)
}

// ReadSnapshotJournal retrieves the serialized in-memory diff layers saved at
// the last shutdown. The blob is expected to be max a few 10s of megabytes.
func ReadSnapshotJournal(db marsdb.KeyValueReader) []byte {
...
func WriteSnapshotJournal(db marsdb.KeyValueWriter, journal []byte) {
...
// the last shutdown
func DeleteSnapshotJournal(db marsdb.KeyValueWriter) {
...
// the last shutdown.
func ReadSnapshotGenerator(db marsdb.KeyValueReader) []byte {
...
// shutdown.
func WriteSnapshotGenerator(db marsdb.KeyValueWriter, generator []byte) {
...
// the last shutdown
func DeleteSnapshotGenerator(db marsdb.KeyValueWriter) {
...
// snapshot layer.
func ReadSnapshotRecoveryNumber(db marsdb.KeyValueReader) *uint64 {
...
// snapshot layer.
func WriteSnapshotRecoveryNumber(db marsdb.KeyValueWriter, number uint64) {
	var buf [8]byte
...
// snapshot layer.
func DeleteSnapshotRecoveryNumber(db marsdb.KeyValueWriter) {
...
func ReadSnapshotSyncStatus(db marsdb.KeyValueReader) []byte {
	data, _ := db.Get(snapshotSyncStatusKey)
	return data
}

// WriteSnapshotSyncStatus stores the serialized sync status to save at shutdown.
func WriteSnapshotSyncStatus(db marsdb.KeyValueWriter, status []byte) {
