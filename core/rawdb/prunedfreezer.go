...
type prunedfreezer struct {
	db marsdb.KeyValueStore // Meta database
...
func newPrunedFreezer(datadir string, db marsdb.KeyValueStore, offset uint64) (*prunedfreezer, error) {
...
func (f *prunedfreezer) ReadAncients(fn func(marsdb.AncientReader) error) (err error) {
	return fn(f)
}
...
func (f *prunedfreezer) ModifyAncients(func(marsdb.AncientWriteOp) error) (int64, error) {
