...
func NewPruner(db marsdb.Database, datadir, trieCachePath string, bloomSize, triesInMemory uint64) (*Pruner, error) {
...
func NewBlockPruner(db marsdb.Database, n *node.Node, oldAncientPath, newAncientPath string, BlockAmountReserved uint64) *BlockPruner {
	return &BlockPruner{
		db:                  db,
...
func NewAllPruner(db marsdb.Database) (*Pruner, error) {
	headBlock := rawdb.ReadHeadBlock(db)
	if headBlock == nil {
		return nil, errors.New("Failed to load head block")
	}
...
func pruneAll(maindb marsdb.Database, g *core.Genesis) error {
	var (
		count  int
...
func prune(snaptree *snapshot.Tree, root common.Hash, maindb marsdb.Database, stateBloom *stateBloom, bloomPath string, middleStateRoots map[common.Hash]struct{}, start time.Time) error {
...
// in the disk.
func RecoverPruning(datadir string, db marsdb.Database, trieCachePath string, triesInMemory uint64) error {
...
func extractGenesis(db marsdb.Database, stateBloom *stateBloom) error {
