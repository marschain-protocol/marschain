...
type Trie interface {
	...
	Prove(key []byte, fromLevel uint, proofDb marsdb.KeyValueWriter) error
}

// NewDatabase creates a backing store for state. The returned database is safe for
// concurrent use, but does not retain any recent trie nodes in memory. To keep some
// historical state in memory, use the NewDatabaseWithConfig constructor.
func NewDatabase(db marsdb.Database) Database {
...
// large memory cache.
func NewDatabaseWithConfig(db marsdb.Database, config *trie.Config) Database {
...
func NewDatabaseWithConfigAndCache(db marsdb.Database, config *trie.Config) Database {
	csc, _ := lru.New(codeSizeCacheSize)
	cc, _ := lru.New(codeCacheSize)
