...
type BlockChain struct {
	...
	db         marsdb.Database // Low level persistent database to store final content in
...
// Processor.
func NewBlockChain(db marsdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine,
...
func (bc *BlockChain) setHeadBeyondRoot(head uint64, root common.Hash, repair bool) (uint64, error) {
	...
	updateFn := func(db marsdb.KeyValueWriter, header *types.Header) (uint64, bool) {
	...
	delFn := func(db marsdb.KeyValueWriter, hash common.Hash, num uint64) {
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			if batch.ValueSize() > marsdb.IdealBatchSize || i == len(blockChain)-1 {
		...
		if bc.cacheConfig.TrieDirtyDisabled {
			...
			if current := block.NumberU64(); current > bc.triesInMemory {
				...
				if nodes > limit || imgs > 4*1024*1024 {
					triedb.Cap(limit - marsdb.IdealBatchSize)
				}
	...
	for {
		...
		case <-recheck.C:
			...
			for !bc.diffQueue.Empty() {
				...
				if batch != nil && batch.ValueSize() > marsdb.IdealBatchSize {
					if err := batch.Write(); err != nil {
