...
func (w *worker) newWorkLoop(recommit time.Duration) {
	...
	for {
		...
		case head := <-w.chainHeadCh:
			...
			timestamp = time.Now().Unix()
			if p, ok := w.engine.(*deimos.Deimos); ok {
		...
		case <-timer.C:
			...
			if w.isRunning() && ((w.chainConfig.Ethash != nil) || (w.chainConfig.Clique != nil &&
				w.chainConfig.Clique.Period > 0) || (w.chainConfig.Deimos != nil && w.chainConfig.Deimos.Period > 0)) {
...
func (w *worker) mainLoop() {
	...
	for {
		...
		case ev := <-w.chainSideCh:
			...
			if _, ok := w.engine.(*deimos.Deimos); ok {
				continue
			}
...
func (w *worker) resultLoop() {
	...
	for {
		select {
		case block := <-w.resultCh: //after sealing a block, it will be sent to resultCh
