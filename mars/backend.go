...
// Package mars implements the Ethereum protocol.
package mars
...
import (
	...
	"github.com/ethereum/go-ethereum/mars/ethconfig"
	"github.com/ethereum/go-ethereum/mars/filters"
	"github.com/ethereum/go-ethereum/mars/gasprice"
...
type Ethereum struct {
	...
	// DB interfaces
	chainDb marsdb.Database // Block chain database
...
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	if config.SyncMode == downloader.LightSync {
		return nil, errors.New("can't run mars.Ethereum in light sync mode, use les.LightEthereum")
	}
	...
	chainDb, err := stack.OpenAndMergeDatabase("chaindata", config.DatabaseCache, config.DatabaseHandles,
		config.DatabaseFreezer, config.DatabaseDiff, "mars/db/chaindata/", false, config.PersistDiff, config.PruneAncientData)
	if err != nil {
		return nil, err
	}
...
func (s *Ethereum) shouldPreserve(header *types.Header) bool {
	...
	if _, ok := s.engine.(*deimos.Deimos); ok {
		return false
	}
...
func (s *Ethereum) StartMining(threads int) error {
	...
	if !s.IsMining() {
		...
		if deimos, ok := s.engine.(*deimos.Deimos); ok {
			wallet, err := s.accountManager.Find(accounts.Account{Address: eb})
			if wallet == nil || err != nil {
				log.Error("Etherbase account unavailable locally", "err", err)
				return fmt.Errorf("signer missing: %v", err)
			...
			deimos.Authorize(eb, wallet.SignData, wallet.SignTx)
		}
...
func (s *Ethereum) ChainDb() marsdb.Database           { return s.chainDb }
