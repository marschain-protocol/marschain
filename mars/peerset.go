...
import (
	...
	"github.com/ethereum/go-ethereum/mars/downloader"
	"github.com/ethereum/go-ethereum/mars/protocols/diff"
	"github.com/ethereum/go-ethereum/mars/protocols/eth"
	"github.com/ethereum/go-ethereum/mars/protocols/snap"
	"github.com/ethereum/go-ethereum/mars/protocols/trust"
...
var (
	...
	// snap protocol without advertising the mars main protocol.
	errSnapWithoutEth = errors.New("peer connected on snap without compatible mars support")

	// errDiffWithoutEth is returned if a peer attempts to connect only on the
	// diff protocol without advertising the mars main protocol.
	errDiffWithoutEth = errors.New("peer connected on diff without compatible mars support")

	// errTrustWithoutEth is returned if a peer attempts to connect only on the
	// trust protocol without advertising the mars main protocol.
	errTrustWithoutEth = errors.New("peer connected on trust without compatible mars support")
)
...
// the `mars` protocol, with or without the `snap` extension.
type peerSet struct {
	peers     map[string]*ethPeer // Peers connected on the `mars` protocol
	snapPeers int                 // Number of `snap` compatible peers for connection prioritization

	snapWait map[string]chan *snap.Peer // Peers connected on `mars` waiting for their snap extension
	snapPend map[string]*snap.Peer      // Peers connected on the `snap` protocol, but not yet on `mars`

	diffWait map[string]chan *diff.Peer // Peers connected on `mars` waiting for their diff extension
	diffPend map[string]*diff.Peer      // Peers connected on the `diff` protocol, but not yet on `mars`

	trustWait map[string]chan *trust.Peer // Peers connected on `mars` waiting for their trust extension
	trustPend map[string]*trust.Peer      // Peers connected on the `trust` protocol, but not yet on `mars`
...
// registerSnapExtension unblocks an already connected `mars` peer waiting for its
// `snap` extension, or if no such peer exists, tracks the extension for the time
// being until the `mars` main protocol starts looking for it.
func (ps *peerSet) registerSnapExtension(peer *snap.Peer) error {
	// Reject the peer if it advertises `snap` without `mars` as `snap` is only a
	// satellite protocol meaningful with the chain selection of `mars`
	...
	// Inject the peer into an `mars` counterpart is available, otherwise save for later
...
// registerDiffExtension unblocks an already connected `mars` peer waiting for its
...
func (ps *peerSet) registerDiffExtension(peer *diff.Peer) error {
	// Reject the peer if it advertises `diff` without `mars` as `diff` is only a
	// satellite protocol meaningful with the chain selection of `mars`
	if !peer.RunningCap(eth.ProtocolName, eth.ProtocolVersions) {
...
// registerTrustExtension unblocks an already connected `mars` peer waiting for its
...
func (ps *peerSet) registerTrustExtension(peer *trust.Peer) error {
	// Reject the peer if it advertises `trust` without `mars` as `trust` is only a
	// satellite protocol meaningful with the chain selection of `mars`
	if !peer.RunningCap(eth.ProtocolName, eth.ProtocolVersions) {
		return errTrustWithoutEth
	}
	// If the peer isn't verify node, don't register trust extension into mars protocol.
	if !peer.VerifyNode() {
		return nil
	}
	// Ensure nobody can double connect
...
// registerPeer injects a new `mars` peer into the working set, or returns an error
...
// len returns if the current number of `mars` peers in the set. Since the `snap`
// peers are tied to the existence of an `mars` connection, that will always be a
// subset of `mars`.
