...
// on the `mars` protocol and convert them into a more easily testable form.
...
func (h *testEthHandler) Handle(peer *eth.Peer, packet eth.Packet) error {
	...
	default:
		panic(fmt.Sprintf("unexpected mars packet type in tests: %T", packet))
	}
}
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	for len(seen) < len(insert) {
		...
		case 66:
			...
			case <-bcasts:
				t.Errorf("initial tx broadcast received on post mars/66")
			}
...
// Tests that post mars protocol handshake, clients perform a mutual checkpoint
