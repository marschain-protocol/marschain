...
type Config struct {
	...
	// RPCGasCap is the global gas cap for mars-call variants.
	RPCGasCap uint64

	// RPCEVMTimeout is the global timeout for mars-call.
...
func CreateConsensusEngine(stack *node.Node, chainConfig *params.ChainConfig, config *ethash.Config, notify []string, noverify bool, db marsdb.Database, ee *ethapi.PublicBlockChainAPI, genesisHash common.Hash) consensus.Engine {
	if chainConfig.Deimos != nil {
		return deimos.New(chainConfig, db, ee, genesisHash)
	}
