...
func Register(stack *node.Node, backend *mars.Ethereum) error {
...
func NewConsensusAPI(eth *mars.Ethereum) *ConsensusAPI {
...
func (api *ConsensusAPI) ExecutePayloadV1(params beacon.ExecutableDataV1) (beacon.ExecutePayloadResponse, error) {
	...
	if !api.eth.BlockChain().HasBlock(block.ParentHash(), block.NumberU64()-1) {
		/*
			...
			if err := api.mars.Downloader().BeaconSync(api.mars.SyncMode(), block.Header()); err != nil {
				return SYNCING, err
			}
		*/
