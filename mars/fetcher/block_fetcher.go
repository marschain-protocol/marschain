...
var (
	blockAnnounceInMeter   = metrics.NewRegisteredMeter("mars/fetcher/block/announces/in", nil)
	blockAnnounceOutTimer  = metrics.NewRegisteredTimer("mars/fetcher/block/announces/out", nil)
	blockAnnounceDropMeter = metrics.NewRegisteredMeter("mars/fetcher/block/announces/drop", nil)
	blockAnnounceDOSMeter  = metrics.NewRegisteredMeter("mars/fetcher/block/announces/dos", nil)

	blockBroadcastInMeter   = metrics.NewRegisteredMeter("mars/fetcher/block/broadcasts/in", nil)
	blockBroadcastOutTimer  = metrics.NewRegisteredTimer("mars/fetcher/block/broadcasts/out", nil)
	blockBroadcastDropMeter = metrics.NewRegisteredMeter("mars/fetcher/block/broadcasts/drop", nil)
	blockBroadcastDOSMeter  = metrics.NewRegisteredMeter("mars/fetcher/block/broadcasts/dos", nil)

	headerFetchMeter = metrics.NewRegisteredMeter("mars/fetcher/block/headers", nil)
	bodyFetchMeter   = metrics.NewRegisteredMeter("mars/fetcher/block/bodies", nil)

	headerFilterInMeter  = metrics.NewRegisteredMeter("mars/fetcher/block/filter/headers/in", nil)
	headerFilterOutMeter = metrics.NewRegisteredMeter("mars/fetcher/block/filter/headers/out", nil)
	bodyFilterInMeter    = metrics.NewRegisteredMeter("mars/fetcher/block/filter/bodies/in", nil)
	bodyFilterOutMeter   = metrics.NewRegisteredMeter("mars/fetcher/block/filter/bodies/out", nil)
)
...
type BlockFetcher struct {
	...
	fetchingHook       func([]common.Hash)               // Method to call upon starting a block (mars/61) or header (mars/62) fetch
	completingHook     func([]common.Hash)               // Method to call upon starting a block body fetch (mars/62)
	importedHook       func(*types.Header, *types.Block) // Method to call upon successful header or block import (both mars/61 and mars/62)
}
