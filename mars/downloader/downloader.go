...
var (
	...
	maxQueuedHeaders            = 32 * 1024                         // [mars/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	stateDB marsdb.Database // Database to state sync into (and deduplicate via)
...
func New(checkpoint uint64, stateDb marsdb.Database, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn, options ...DownloadOption) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
