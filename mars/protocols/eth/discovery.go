...
// enrEntry is the ENR entry which advertises `mars` protocol on the discovery.
...
// StartENRUpdater starts the `mars` ENR updater loop, which listens for chain
...
// currentENREntry constructs an `mars` ENR entry based on the current state of the chain.
