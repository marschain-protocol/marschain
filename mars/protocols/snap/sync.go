...
type accountTask struct {
	...
	genBatch marsdb.Batch    // Batch used by the node generator
...
type Syncer struct {
	db marsdb.KeyValueStore // Database to store the trie nodes into (and dedup)
	...
	stateWriter        marsdb.Batch       // Shared batch writer used for persisting raw states
...
// snap protocol.
func NewSyncer(db marsdb.KeyValueStore) *Syncer {
	return &Syncer{
		db: db,
...
func (s *Syncer) loadSyncStatus() {
	...
	if status := rawdb.ReadSnapshotSyncStatus(s.db); status != nil {
		if err := json.Unmarshal(status, &progress); err != nil {
			...
			for _, task := range s.tasks {
				task.genBatch = marsdb.HookedBatch{
					Batch: s.db.NewBatch(),
				...
				for _, subtasks := range task.SubTasks {
					for _, subtask := range subtasks {
						subtask.genBatch = marsdb.HookedBatch{
							Batch: s.db.NewBatch(),
							OnPut: func(key []byte, value []byte) {
...
func (s *Syncer) processStorageResponse(res *storageResponse) {
	...
	if res.subTask != nil {
		...
		if res.subTask.genBatch.ValueSize() > marsdb.IdealBatchSize || res.subTask.done {
...
func (s *Syncer) forwardAccountTask(task *accountTask) {
	...
	if task.genBatch.ValueSize() > marsdb.IdealBatchSize || task.done {
		if err := task.genBatch.Write(); err != nil {
...
func (s *Syncer) onHealState(paths [][]byte, value []byte) error {
	...
	if s.stateWriter.ValueSize() > marsdb.IdealBatchSize {
