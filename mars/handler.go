...
import (
	...
	"github.com/ethereum/go-ethereum/mars/fetcher"
...
type handlerConfig struct {
	Database   marsdb.Database           // Database for direct sync insertions
...
// runEthPeer registers an mars peer into the joint mars/snap peerset, adds it to
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	if err := h.downloader.RegisterPeer(peer.ID(), peer.Version(), peer); err != nil {
		peer.Log().Error("Failed to register peer in mars syncer", "err", err)
		return err
	}
	if snap != nil {
...
// runSnapExtension registers a `snap` peer into the joint mars/snap peerset and
// starts handling inbound messages. As `snap` is only a satellite protocol to
// `mars`, all subsystem registrations and lifecycle management will be done by
// the main `mars` handler to prevent strange races.
...
// runDiffExtension registers a `diff` peer into the joint mars/diff peerset and
...
// runTrustExtension registers a `trust` peer into the joint mars/trust peerset and
...
func (h *handler) unregisterPeer(id string) {
	...
	// Remove the `mars` peer if it exists
